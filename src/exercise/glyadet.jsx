...
import { glyadetEx31, glyadetAnalysis30, glyadetDialog32 } from './glyadet_data.js';
...
export default function GlyadetExercise() {
  ...
  return (
    <div className="exercise-wrapper">
      ...
      {tab === 'quiz' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Bài tập trắc nghiệm - ГЛЯДЕТЬ</h3>
          </div>
          {glyadetEx31.map((q, i) => <QuizQuestion key={q.id} q={q} index={i} />)}
        </div>
      )}
      ...
      {tab === 'analysis' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Bài tập phân tích - ГЛЯДЕТЬ</h3>
          </div>
          {glyadetAnalysis30.map((item, i) => <AnalysisItem key={item.id} item={item} index={i} />)}
        </div>
      )}
      ...
      {tab === 'dialog' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Soạn hội thoại - ГЛЯДЕТЬ</h3>
          </div>
          {glyadetDialog32.map((item, i) => <AnalysisItem key={item.id} item={item} index={i} />)}
        </div>
      )}
    </div>
  );
