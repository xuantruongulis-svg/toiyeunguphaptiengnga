...
import { prichastieExercises } from '../data/ex_prichastie';

const PrichastieExercise = () => {
  ...
  return (
    <div className="practice-grid-container">
      {prichastieExercises.length > 0 ? (
        <div className="grid">
          {prichastieExercises.map((ex, index) => {
        ...
        <div className="placeholder-content" style={{ textAlign: 'center', padding: '4rem 2rem', background: 'rgba(255,255,255,0.05)', borderRadius: '20px', border: '1px dashed rgba(255,255,255,0.2)' }}>
          ...
          <p style={{ color: 'var(--text-secondary)', fontSize: '1.1rem' }}>Các bài tập về Phân từ sẽ sớm được bổ sung tại đây.</p>
        </div>
      )}
    </div>
  );
...
export default PrichastieExercise;
