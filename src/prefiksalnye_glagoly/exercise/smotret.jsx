...
import { smotretEx27_28, smotretAnalysis24_25, smotretReplacement26, smotretDialog29 } from './smotret_data.js';
...
export default function SmotretExercise() {
  ...
  return (
    <div className="exercise-wrapper">
      ...
      {tab === 'quiz' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Bài tập trắc nghiệm - СМОТРЕТЬ</h3>
          </div>
          {smotretEx27_28.map((q, i) => <QuizQuestion key={q.id} q={q} index={i} />)}
        </div>
      )}
      ...
      {tab === 'analysis' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Bài tập phân tích - СМОТРЕТЬ</h3>
          </div>
          {smotretAnalysis24_25.map((item, i) => <AnalysisItem key={item.id} item={item} index={i} />)}
        </div>
      )}
      ...
      {tab === 'replacement' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Bài tập thay thế - СМОТРЕТЬ</h3>
          </div>
          {smotretReplacement26.map((item, i) => <AnalysisItem key={item.id} item={item} index={i} />)}
        </div>
      )}
      ...
      {tab === 'dialog' && (
        <div className="ex-section">
          <div className="ex-section-header">
            <h3>Soạn hội thoại - СМОТРЕТЬ</h3>
          </div>
          {smotretDialog29.map((item, i) => <AnalysisItem key={item.id} item={item} index={i} />)}
        </div>
      )}
    </div>
  );
